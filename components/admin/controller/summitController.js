...
const getSummit = async(req,res)=>{
    await Summit.find({event_id:req.params.event_id},function(err, userObj){ 
        if(err){
...
const editSummit = (req,res)=>
{
    ...
    let event_id= req.params.event_id;
    Summit.findOne({_id:id},function(err, user)
    { 
       if(err)
       {
...
////////////////////////////////////////////////////////////
    ...
    var data = { description:req.body.description,point:req.body.point};
    Summit.findByIdAndUpdate({ _id: id },data,{new: true}, function(err, result) {
    if (err) {
        $message ={message:'Something went wrong'}
        req.flash("errors",$message)
        return res.redirect('/edit-summit/'+id+'/'+event_id);
